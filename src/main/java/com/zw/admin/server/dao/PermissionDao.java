...
public interface PermissionDao {
	...
	@Insert("insert into sys_permission(parentId, name, css, href, type, permission, sort,path,component) values(#{parentId}, #{name}, #{css}, #{href}, #{type}, #{permission}, #{sort},#{path},#{component})")
	int save(Permission permission);

	@Update("update sys_permission t set parentId = #{parentId}, name = #{name}, css = #{css}, href = #{href}, type = #{type}, permission = #{permission}, sort = #{sort},path=#{path},component=#{component} where t.id = #{id}")
