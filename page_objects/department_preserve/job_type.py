...
job_type_map = map_path + "/department_preserve/job_type.xml"
...
class JobType():
    def __init__(self, driver):
        self.driver = driver
        self.job_type = ObjectMap(job_type_map)
        self.job_type_page = NavigateBar(self.driver)


    def add_job_type(self,name):
        self.job_type_page.go_to_job_type()
        self.driver.implicitly_wait(20)
        new_additional = self.job_type.getLocator(self.driver, "New_Additional")
        new_additional.click()
        time.sleep(1)
        job_name = self.job_type.getLocator(self.driver, "Name")
        job_name.send_keys(name)
        ensure = self.job_type.getLocator(self.driver, "Ensure")
        ensure.click()
        try:
            tips = self.job_type.getLocator(self.driver, 'Tips')
    ...
    def close_login_tips(self):
        ...
        关闭登录成功提示
        """
        close_tips = self.job_type.getLocator(self.driver, 'CloseTips')
        close_tips.click()
        time.sleep(1)

    def find_job_type(self,name):
        """
        查找岗位小类是否在列表中
        :param name: 岗位小类
        :return: True/Flase
        ...
        next_page = self.job_type.getLocator(self.driver, "Next_Page")
        name_list = []
        flag = True
        status = 0
        while flag:
